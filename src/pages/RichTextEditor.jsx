...
export default function RichTextEditor() {
  ...
  const [value, setValue] = useState(blogPrompt);
  ...
  useEffect(() => {
    if (topic && keywords && editorRef.current) {
      editorRef.current.value = blogPrompt
    }
  }, [blogPrompt]);
  ...
  return (
    <div className="flex flex-col min-h-screen p-5 bg-gradient-to-r from-purple-300 to-emerald-300">
      ...
      /> */}
      <div className="fixed px-5 py-3 text-lg text-center text-white -translate-x-1/2 -translate-y-1/2 bg-black rounded-md w-max left-1/2 top-1/2"> API key expired 😭 <br /> Don't bother trying </div>
      <textarea
        className="w-full h-20 p-2 border border-gray-300 rounded shadow-xl resize-none"
